...
try:
    ...
    from src.cells import build_cells, assign_cells
except ImportError:
...
def parse_args() -> argparse.Namespace:
    p = argparse.ArgumentParser(description="KPlaceNet L0/L1 train — 4050-safe")
...
def main() -> None:
    ...
    print(f"[train] dataset: {len(ds)} images from {args.csv}")
    ...
    # Effective classes = len(cells); when N < K the builder returns fewer
    # cells, so the head must match len(cells) — not args.num_cells — or
    # eval would index centroids out of range on unsupervised logits.
    ...
    ds.cell_ids = cell_ids  # attach to dataset for __getitem__
    ...
    print(f"[train] trainable params: {sum(p.numel() for p in trainable):,} / {sum(p.numel() for p in model.parameters()):,}")
